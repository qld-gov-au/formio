...
export default ({ form, formConfirmation }) => {
  ...
  form.on("submitDone", (submissionData) => {
    ...
    console.log(submissionData.metadata.PostToAPIGateway.DownloadUrl, 'submissionData.metadata.PostToAPIGateway.DownloadUrl');
    const pdfdownload = new PdfDownload();
    if(pdfdownload.isPdfDownloadEnabled()){
