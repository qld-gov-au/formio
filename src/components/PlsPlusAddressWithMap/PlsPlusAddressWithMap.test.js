...
const functionTest = (caseName) => async () => {
  ...
  const autocomplete = form.querySelector(
    "input[name='data[plsplusaddresswithmap]']"
  );
  ...
  await userEvent.type(
    form.querySelector("input[name='data[plsplusaddresswithmap]']"),
  ...
  expect(
    form.querySelector("input[name='data[plsplusaddresswithmap]']")
  ).toHaveValue(fixtures[`autocompleteInput${caseName}`]);
  ...
  const address1 = form.querySelector(
    "input[name='data[plsplusaddresswithmap][addressData][address1]']"
  );
...
// Smoke test
test("PlsPlusAddressWithMap is rendered", async () => {
  const form = createForm({
    form: formSettings,
  });
  document.body.append(form);
  ...
  await testWait();
  expect(
    form.querySelector("input[name='data[plsplusaddresswithmap][address1]']")
  ).toBeNull();
  ...
  const address2 = form.querySelector(
    "input[name='data[plsplusaddresswithmap][addressData][address2]']"
  );
  expect(address2).toBeVisible();
  const address3 = form.querySelector(
    "input[name='data[plsplusaddresswithmap][addressData][address3]']"
  );
  expect(address3).toBeVisible();
  const city = form.querySelector(
    "input[name='data[plsplusaddresswithmap][addressData][city]']"
  );
  expect(city).toBeVisible();
  const state = form.querySelector(
    "input[name='data[plsplusaddresswithmap][addressData][state]']"
  );
  expect(state).toBeVisible();
  const postcode = form.querySelector(
    "input[name='data[plsplusaddresswithmap][addressData][postcode]']"
  );
...
// Functional test
test(
  "PlsPlusAddressWithMap autocomplete mode is functional",
  functionTest("Case1")
);

// Functional test
test(
  "PlsPlusAddressWithMap autocomplete mode with multiple parseAddress results is functional",
  functionTest("Case2")
);

// Functional test
test("PlsPlusAddressWithMap remove button is functional", async () => {
  ...
  expect(
    form.querySelector(
      "input[name='data[plsplusaddresswithmap][address][address1]']"
    )
  ).toBeNull();
  expect(
    form.querySelector("input[name='data[plsplusaddresswithmap]']")
...
// Functional test
test("PlsPlusAddressWithMap manual mode is functional", async () => {
  let data = {};
  const form = createForm({
    form: formSettings,
    controller: (formio) => {
...
// Functional test
test("PlsPlusAddressWithMap with custom address sub-field keys", async () => {
  let data = {};
  const form = createForm({
    form: {
      components: [
